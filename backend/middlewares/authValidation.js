...
let validateUpdateUser=(data)=>{
    if(data.fullName !== undefined && !/^[a-zA-Z ]+$/.test(data.fullName)){
        return {status:false,message:"Name does not contains numbers or special Symbols"}
    }
    if(data.username !== undefined && !/^[a-zA-Z][a-zA-Z0-9_]{4,19}$/.test(data.username)){
        return{status:false,message:"username doesn't meet with standard guidelines"}
    }
    if(data.password !== undefined && data.password !== '' && !/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(data.password)){
        return{status:false,message:"password doesn't meet with standard guidelines"}
    }
